...
void sendMessage(IOTHUB_DEVICE_CLIENT_LL_HANDLE *device_ll_handle, CLIENT_SAMPLE_INFO *user_ctx)
{
    ...
    if(result == IOTHUB_CLIENT_OK){
        (void)printf("successful set message callback\r\n");
    } else {
        (void)printf("error\r\n");
        // Optionally, you can print the error code
        (void)printf("Error code: %d\r\n", result);
    ...
    (void)printf("iothub: %d, %d", iothub_info.connected, iothub_info.stop_running);

    (void)printf("Sending 1 message to IoTHub every %d seconds for %d messages (Send any message to stop)\r\n", TIME_BETWEEN_MESSAGES, MESSAGES_TO_SEND);
    do
    {
        ...
        ThreadAPI_Sleep(1);
    } while (iothub_info.stop_running == 0 && msg_count < 2);

    (void)printf("after IoTHubDeviceClient_LL_DoWork\r\n");
